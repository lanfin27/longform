...
with tab2:
    ...
    **씬 분석이란?**
    ...
    if analysis_path.exists():
        ...
        with col2:
            st.metric("캐릭터 수", len(saved_analysis.get("characters", [])))
        with col3:
        ...
        for i, scene in enumerate(scenes):
            ...
            script_preview = scene.get('script_text', '')[:50]

            with st.expander(f"씬 {scene_id}: {script_preview}...", expanded=False):
                col_left, col_right = st.columns([1, 1])

                with col_left:
                    st.markdown("**📝 스크립트**")
                    st.write(scene.get("script_text", ""))

                    st.markdown("**🎬 연출가이드**")
                    st.info(scene.get("direction_guide", ""))
                ...
                with col_right:
                    st.markdown("**🎨 이미지 프롬프트 (EN)**")
                    st.code(scene.get("image_prompt_en", ""), language=None)
    ...
    with prompt_tab1:
        ...
        with col_list:
            ...
            with st.popover("➕ 새 템플릿 추가"):
                new_t_name = st.text_input("템플릿 이름", placeholder="예: 다큐멘터리 스타일")
                new_t_desc = st.text_input("설명", placeholder="다큐멘터리 영상을 위한 상세 분석")
